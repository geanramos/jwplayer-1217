<?php
	...
	for ($i = 0; $i < $location_count; $i++) {
		...
		$response = $client->request('GET', 'http://www.webpagetest.org/runtest.php', [
		    'query' => [
				'url' 		=> 'http://jwplayer-1217.appspot.com/flash',  	// url for FLASH
				'k'   		=>  'A.2eac1a1250a8fb22d3459e67726a76e5',		// API key
				'video' 	=>  '1',										// capture video
				'fvonly'	=>  '1',										// first view only
				'location'	=>  $locations[$i] 								// the webpagetest.org POP
			]
		]);
	...
	for ($i = 0; $i < $location_count; $i++) {
		...
		$response = $client->request('GET', 'http://www.webpagetest.org/runtest.php', [
		    'query' => [
				'url' 		=> 'http://jwplayer-1217.appspot.com/html5', // url for HTML5
